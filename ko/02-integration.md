...
현재는 Intel/AMD64 CPU와 OSX 또는 Linux만 지원합니다. Linux의 경우 표준 빌드 명령은 `glibc` 시스템(Ubuntu, Debian, CentOS 등)에서 작동합니다. 알파인과 같은 `muslc` 기반 시스템용으로 컴파일하려면 정적 라이브러리 wasmvm을 로컬에서 컴파일하고 `muslc` 빌드 태그와 함께 go를 컴파일해야 합니다. 아니면 알파인 시스템에서 정적 이동 바이너리를 빌드하는 [Dockerfile](./Dockerfile) 을 사용하십시오.
...
위 설명처럼 하기 위해 커스텀 Cosmos SDK 앱을 사용하고 싶다면 *가장 추천하는 방법은* 간단히 `wasmd` 포크하는 것 입니다. 아래 설명하는 방법 중 하나를 시도해 보세요.
...
그러나 때로는 `x/wasm` 을 앱에 복사해야 합니다. 이것은 제한된 경우에 해야 하며 업그레이드가 더 어려워지므로 가능하면 위의 경로를 따르십시오. 복사하는 방법은 앱에서 일부 주요 SDK 모듈을 비활성화한 경우(예: 스테이킹이 아닌 PoA를 사용하고 이러한 콜백 및 기능 지원을 비활성화 함), 혹은 Cosmos SDK에서 코어 `x/*` 모듈을 복사하고 어떻게 든 응용 프로그램을 커스터마이즈한 경우에 필요합니다.
...
클로저를 통해 `Keeper` 를 전달하는 것을 포함하여 중요한 경우에 이를 빌드하는 방법을 보고 싶다면 `staking` 모듈의 구현을 볼 수 있습니다. 여기에서 우리는 [스테이킹 메시지를 인코딩합니다](https://github.com/CosmWasm/wasmd/blob/v0.8.0-rc1/x/wasm/internal/keeper/handler_plugin.go#L114-L192) . 철회는 2개의 메시지를 반환하며, 이는 필요한 경우 기본 메시지로 번역하는 데 사용할 수 있는 옵션입니다. [스테이킹 쿼리](https://github.com/CosmWasm/wasmd/blob/v0.8.0-rc1/x/wasm/internal/keeper/query_plugins.go#L109-L172)를 처리할 때 `Keeper in the closure` 가져오고 계약에서 기본 `Keeper` 인터페이스로 사용자 지정 `QueryRequest` 를 전달한 다음 응답을 인코딩합니다. 반환 유형을 정의할 때 Rust 계약에서 적절한 구문 분석을 위해 Rust가 `Option<T>` 를 예상하는 경우 JSON 필드의 이름을 올바르게 지정하고 `omitempty` 키워드를 사용해야 합니다. 또한 Rust `enum` 에 해당하는 모든 필드에 대해 `omitempty` 및 포인터를 사용하기 때문에 정확히 하나의 필드로 직렬화됩니다.
