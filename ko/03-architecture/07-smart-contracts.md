...
이더리움처럼, 컨트랙트 인스턴스화 및 실행은 계량이 되고 가스가 필요합니다. 추가로, 인스턴스화 및 실행은 서명자가 컨트랙트에 토큰을 보낼 때 메시지도 함께 보낼 수 있게 합니다. 주요 차이점은 `SendMsg`를 이용하는 것 같이 컨트랙트로 토큰을 직접 보내는 것은 가능하면, *어떠한 컨트랙트 코드도 실행시키지 않습니다*. 가능한 공격 방법을 줄이기 위한 깔끔한 설계인 것입니다. 그 어떤 것도 불가능하게 만들지 않지만, 모든 컨트랙트의 실행은 *명시적으로 요청받아야 합니다*.
...
Cosmwasm은 컨트랙트가 다른 컨트랙트를 직접 호출하는 것을 막아 재진입을 방지합니다. 컴포지션을 활용하고 싶지만, 잘못된 코드에 대한 인라인 함수 호출은 보안에 큰 위협이 될 수 있습니다. CosmWasm에서 취하는 접근법은 메시지 목록을 *반환*하는 컨트랙트들은 *같은 트랜잭션에서 실행이 되도록* 하는 것입니다. 컨트랙트는 완료가 된 이후에 전송을 요청하거나 (예: 에스크로 릴리스), 다른 컨트랙트를 호출할 수 있음을 의미합니다. 만약 이후 메시지가 실패한다면, 업데이트가 된 컨트랙트의 상태를 포함 전체 트랜잭션은 복구가 됩니다. 이는 원자성 있는 컴포지션과 상당한 보안을 보장할 수 있으며, 단 하나의 단점은 다른 컨트랙트의 실행 결과를 볼 수 없고 "오류 시 되돌리기"만 할 수 있습니다.
...
블록 해시들(과 타임 스탬프들의 마지막 자리들) 이더리움의 채굴자보다 텐더민트의 블록 제안자들에 의해 훨씬 쉽게 조작됩니다. 이것들은 무작위성을 위해 사용되면 안됩니다. 안전한 랜덤 비콘 체인을 제공하고 스마트 컨트랙트로의 안전한 엔트로피의 근원을 제공하는 계획이 있습니다.
