...
서브메시지 `execution` 및 `reply` 은 다른 서브메시지의 컨텍스트 내에서 발생할 수 있다는 것을 기억하십시오. 예를 들어, `contract-A--submessage --> contract-B--submessage --> contract-C`의 상황을 보겠습니다. 여기에서 `contract-B` 는 서브메시지 `reply`에서 `Err`을 리턴하여 `contract-C` 및 자체의 상태를 되돌릴 수 있지만, 컨트랙트 A 또는 전체 트랜잭션을 되돌릴 수는 없습니다. 단지 컨트랙트 A의 `reply` 기능으로 `Err`을 반환하는 것으로 끝납니다.
